...
const bundle: Record<Lang, Record<string, string>> = {
  tg: {
    ...
    'usercard.rebuildConfirm':
      'Маҷмӯи задачаҳоро аз рӯи плани ҷорӣ аз нав созам? ҲАМАИ задачаҳои худкор (ҳатто ишорашуда) аз нав сохта мешаванд — ишораҳо гум мешаванд. Танҳо задачаҳои дастӣ таъиншуда мемонанд.',
    'usercard.rebuildDone': 'Маҷмӯъ аз нав сохта шуд: {n} задача',
    'usercard.subOrderError': 'Санаи «Обуна то» аз «Обуна аз» пештар аст — санаҳоро санҷед',
  ...
  ru: {
    ...
    'usercard.rebuildConfirm':
      'Пересоздать набор задач по текущему плану? ВСЕ авто-задачи (включая отмеченные) будут созданы заново — отметки выполнения сбросятся. Останутся только задачи, назначенные вручную.',
    'usercard.rebuildDone': 'Набор пересоздан: {n} задач',
    'usercard.subOrderError': 'Дата «Подписка по» раньше даты «Подписка с» — проверьте даты',
    // Метка плана подписки (0051)

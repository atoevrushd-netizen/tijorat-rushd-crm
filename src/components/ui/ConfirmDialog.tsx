...
export function ConfirmDialog() {
  ...
  const titleId = useId()
  const msgId = useId()
  // a11y: фокус внутрь (первая кнопка = «Отмена», безопасный выбор), ловушка Tab, возврат фокуса.
  useFocusTrap(panelRef, !!req)
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="alertdialog"
        aria-modal="true"
        aria-labelledby={o.title ? titleId : undefined}
      ...
      >
        {o.title && <h2 id={titleId} className="mb-1.5 text-[17px] font-bold text-ink">{o.title}</h2>}
        <p id={msgId} className="whitespace-pre-line text-sm text-ink-2">{o.message}</p>

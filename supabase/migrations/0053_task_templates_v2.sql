...
begin
  if p_plan = 6 then
    ...
    if p_idx = 1 then
      ...
      -- Только 2-й месяц 6-месячных: CRM + скрипты продаж + обучение куратором
      return query select * from (values
        ('crm'::text,         'Создание CRM'::text,                       'Сохтани CRM'::text,                          1),
        ('sales_script',      'Создание скриптов продаж',                 'Сохтани скриптҳои фурӯш',                    1),
        ('curator_training',  'Обучение куратором — скриптов продаж',     'Омӯзиши куратор — скриптҳои фурӯш',          1)
      ) as t(typ, ru, tg, cnt);
    end if;
  else
